...
public class l06crossword{
    ...
    public static boolean[] placeH(String word, int r, int c){
        ...
        for(int i=0;i<word.length();i++){
            if(box[r][c+i] == '-'){             // now if theres already a word here we dont need to put effort to place it again
                char ch = word.charAt(i);          // and the is correct as we checked it in second if statement of issafeToplaceH
                box[r][c+i] = ch;
                loc[i] = true;
            }
        }
    ...
    public static boolean[] placeV(String word, int r, int c){
        boolean[] loc = new boolean[word.length()];
        for(int i=0;i<word.length();i++){
            if(box[r+i][c] == '-'){             // now if theres already a word here we dont need to put effort to place it again
                char ch = word.charAt(i);          // and the is correct as we checked it in second if statement of issafeToplaceH
                box[r+i][c] = ch;
                loc[i] = true;
            }
    ...
    public static void crossWordMain(){
